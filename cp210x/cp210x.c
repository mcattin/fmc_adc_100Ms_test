...
static int cp210x_tiocmset(struct tty_struct *, struct file *, unsigned int, unsigned int);
...
static int cp210x_tiocmset (struct tty_struct *tty, struct file *f,
...
static int cp210x_tiocmget (struct tty_struct *tty, struct file *f)
{
	struct usb_serial_port *port = tty->driver_data;
