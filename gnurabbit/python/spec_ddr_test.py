...
if __name__ == '__main__':
    ...
    print("Status register    : %.8X") % (regs.rd_reg(STAT))
    print("Control register 1 : %.8X") % (regs.rd_reg(CTRL_1))
    print("Control register 2 : %.8X") % (regs.rd_reg(CTRL_2))
    print("Control register 3 : %.8X") % (regs.rd_reg(CTRL_3))
    print("LED ctrl register  : %.8X") % (regs.rd_reg(CTRL_LED))

    print("Blink LEDs")
    carrier_led_test(regs)
    ...
    gn4124.add_dma_item(0, pages[1], dma_length, 1, 0) # write from page 1 to SPEC memory
