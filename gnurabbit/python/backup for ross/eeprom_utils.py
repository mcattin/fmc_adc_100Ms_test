...
if __name__ == '__main__':
    ...
    def gennum_dump_to_screen(gennum):
        for i in range(0x0,0xE04,4):
            print("[%.3X]: %.8X") % (i,gennum.iread(4, i, 4))
        return 0

    def gennum_dump_to_file(gennum):
        file_name = raw_input('Enter a file name (default=gn4124.dat) :')
        if file_name == "":
            file_name = "gn4124.dat"
        file = open(file_name, 'w+')
        gennum_data=[]
        for i in range(0x0,0xE04,4):
            data = gennum.iread(4, i, 4)
            print >>file,'%.3X %.8X' %(i,data)
            #print("[%.3X]: %.8X") % (i,data)
        return 0

    def compare_gennum_with_file(gennum):
        file_name = raw_input('Enter a file name (default=gn4124.dat):')
        if file_name == "":
            file_name = "gn4124.dat"
        file = open(file_name, 'r+')
        ...
        # Read EEPROM
        for i in range(0x0,0xE04,4):
            gennum_data.append(gennum.iread(4, i, 4))
        # Compare
        err_cnt = 0
        confirm = raw_input('Do want to save the difference in a file? (Type "Yes" to confirm) :')
        if confirm == "Yes":
            file_name = raw_input('Enter a file name (default=gn4124_diff.dat):')
            if file_name == "":
                file_name = "gn4124_diff.dat"
            file = open(file_name, 'w+')
        
        for i in range(0,len(file_data)):
            if file_data[i] != gennum_data[i]:
                print '[%.3X] GN4124= %.8X, FILE= %.8X => ERROR' %(4*i, gennum_data[i],file_data[i])
                if confirm == "Yes":
                    print >>file,'%.3X %.8X' %(4*i,file_data[i])
                err_cnt += 1
        print ' '
        print '====> %d error(s)' % err_cnt
        print ' '
        return 0

    def file_dump_to_gennum(gennum):
        file_name = raw_input('Enter a file name (default=gn4124_diff.dat):')
        if file_name == "":
            file_name = "gn4124_diff.dat"
        file = open(file_name, 'r+')
        ...
        confirm = raw_input('Do you really want to write the Gennum configuration space? (Type "Yes" to confirm) :')
        if confirm != "Yes":
            return 1
        for i in range(0,len(file_data)):
            gennum.iwrite(4, file_addr[i], 4, file_data[i])
        print ' '
    ...
    def display_options():
        ...
        print '6 -> Read Gennum config and dump on screen.'
        print '7 -> Read Gennum config and dump to file.'
        print '8 -> Verify Gennum config against file.'
        print '9 -> Write Gennum config from file.'
        print '0 -> Exit'
        print ' '
    ...
    def bind(input, gennum):
        ...
        if input == "5":
            ...
            return 0
        if input == "6":
            gennum_dump_to_screen(gennum)
            return 0
        if input == "7":
            gennum_dump_to_file(gennum)
            return 0
        if input == "8":
            compare_gennum_with_file(gennum)
            return 0
        if input == "9":
            file_dump_to_gennum(gennum)
            return 0
        if input == "0":
