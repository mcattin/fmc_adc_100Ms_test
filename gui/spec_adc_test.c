...
int SDL_init(SDL_Surface **screen, TTF_Font **fonts)
{
	...
	*fonts=TTF_OpenFont("/usr/share/fonts/truetype/freefont/FreeSans.ttf", 13 /*font size */);
	if(!(*fonts))
	{
...
int key_check(int key)
{
	if (key == 27) //range - r
	{
		...
		if (params.adc_range == 2)
			adc_set_range(params.adc_channel, ADC_RANGE_10V, params.adc_50ohm);
                */
                fprintf(stderr, "\nChange input voltage range function is not implemented!\n\n");
		return 0;
	}
	...
	if (key == 111) //offset - up
	{
		...
		//adc_set_offset(params.adc_channel, params.adc_offset);
                fprintf(stderr, "\nChange DC offset function is not implemented!\n\n");
		return 0;
	}
	...
	if (key == 28) //termination - t
	{
		...
		if (params.adc_range == 2)
			adc_set_range(params.adc_channel, ADC_RANGE_10V, params.adc_50ohm);
                */
                fprintf(stderr, "\nSet input termination function is not implemented!\n\n");
		return 0;
	}
	...
	if (key == 58) // temp measurement - m
	{
		...
		params.do_temp_meas %=2;
                */
                fprintf(stderr, "\nTemperature measurement function is not implemented!\n\n");
		return 0;
	}
	...
	if ((key == 59) || (key == 60)) // freq mod: < >
	{
		...
		fprintf(stderr, "\nfrequency: %d Hz, %d\n\n", params.base_freq, N1_temp);
                */
                fprintf(stderr, "\nChange sampling frequency function is not implemented!\n\n");
		return 0;
	}
	...
	if (key == 34) // write mem - [
	{
		...
		fprintf(stderr, "\n256 bytes written.\n");
                */
                fprintf(stderr, "\nWrite memory function is not implemented!\n\n");
		return 0;
	}
	...
	if (key == 35) // read mem - ]
	{
		...
		for (i=0;i<256;i++)
		{
			...
			fprintf(stderr, "data: %d\n", temp_char[0]);
                ...
                */
                fprintf(stderr, "\nRead memory function is not implemented!\n\n");
		return 0;
	}
...
main()
{
	...
	int keypress = 0;
        ...
        ch1_file = fopen("../acq/ch1.txt","w");
        ch2_file = fopen("../acq/ch2.txt","w");
        ch3_file = fopen("../acq/ch3.txt","w");
        ch4_file = fopen("../acq/ch4.txt","w");
	...
	while (!keypress)
	{
                if ((50 == meas.no) && (0 == wr_file_flag)){
                        wr_file_flag = 1;
                        adc_record_single(1, buf, NSAMPLES);
                        if (NULL != ch1_file){
                                for(i=0;i<NSAMPLES;i++){
                                        fprintf(ch1_file, "%4d, 0x%.4X\n", i, buf[i]);
                                }
                        }
                        else{
                                fprintf(stderr, "Cannot open %s file in write mode!\n", ch1_file);
                        }
                        adc_record_single(2, buf, NSAMPLES);
                        if (NULL != ch2_file){
                                for(i=0;i<NSAMPLES;i++){
                                        fprintf(ch2_file, "%4d, 0x%.4X\n", i, buf[i]);
                                }
                        }
                        else{
                                fprintf(stderr, "Cannot open %s file in write mode!\n", ch2_file);
                        }
                        adc_record_single(3, buf, NSAMPLES);
                        if (NULL != ch3_file){
                                for(i=0;i<NSAMPLES;i++){
                                        fprintf(ch3_file, "%4d, 0x%.4X\n", i, buf[i]);
                                }
                        }
                        else{
                                fprintf(stderr, "Cannot open %s file in write mode!\n", ch3_file);
                        }
                        adc_record_single(4, buf, NSAMPLES);
                        if (NULL != ch4_file){
                                for(i=0;i<NSAMPLES;i++){
                                        fprintf(ch4_file, "%4d, 0x%.4X\n", i, buf[i]);
                                }
                        }
                        else{
                                fprintf(stderr, "Cannot open %s file in write mode!\n", ch4_file);
                ...
                adc_record_single(params.adc_channel, buf, NSAMPLES);
